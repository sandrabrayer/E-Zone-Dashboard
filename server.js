 ...
 * (identity foundation): trim, strip control characters and angle brackets
 * (defense-in-depth against a name ever being interpolated into HTML), cap at
 * 40 characters. '' (no name) is always allowed — the cookie then keeps the
 * legacy user-less format and everything behaves exactly as before. */
...
app.post('/api/sheets', requireSession, async (req, res) => {
  const body = req.body || {};
  // Identity foundation: the `user` the Apps Script stamps into updatedBy
...
app.post('/api/verify-pin', (req, res) => {
  ...
  if (checkPin(pin, APP_PIN)) {
    ...
    if (SESSION_SECRET) {
      // Optional `user` (identity foundation): a Hebrew display name the
      // login may attach; it rides INSIDE the signed token so it cannot be
      // changed without breaking the HMAC. Absent/empty → the legacy
      // user-less token, byte-for-byte as before.
 ...
 * (identity foundation). Session-gated like every data route; a legacy
 * user-less cookie answers { user: '' } and everything keeps working. The
 * frontend uses this in the follow-up PR (name picker) — no UI reads it yet. */
...
module.exports = {
  ...
  buildSessionCookie,
  requestIsHttps,
  // Identity foundation (see test/patient-identity-foundation.test.js).
  sanitizeSessionUser,
