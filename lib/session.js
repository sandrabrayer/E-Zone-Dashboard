 ...
 *
 * USER-BEARING variant (identity foundation): `<expiry>.<userB64>.<signature>`
 * where userB64 is base64url(UTF-8 user name) and the signature covers
 * "<expiry>.<userB64>" — so the name is tamper-proof (any edit breaks the
 * HMAC) while legacy 2-part tokens keep validating unchanged. The two
 * formats cannot be confused: a legacy signing message ends in digits only,
 * a user-bearing one always contains the '.' + base64url part.
 *
